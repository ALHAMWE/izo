...
import { updatePurchase } from 'src/store/apps/purchases/postUpdatePurchaseSlice'
import { fetchPurchases } from 'src/store/apps/purchases/getPurchasesSlice'
import AlertCheckUpdate from './AlertCheckUpdate'
...
const EditPurchaseCard = ({ id }) => {
  ...
  const updateStatus = useSelector(state => state.postUpdatePurchase)
    ...
    if (store) {
      ...
      setInitialValues(prev => {
        return {
          ...
          old_sts: info[0].status,
          ref_no: requirement?.invoice_number,
          payment_status: info[0].payment_status,
          old_status: info[0].payment_status,
          ...
          contact_id: info[0].contact, // supplier id
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    setOpenLoading(true)
    dispatch(updatePurchase({ values, id })).then(() => {
  ...
  return (
    <Fragment>
      {openLoading && (
        <LoadingAnimation open={openLoading} onClose={() => setOpenLoading(false)} statusType={updateStatus} />
      )}
      ...
      <Formik
        ...
        enableReinitialize
      >
        {({ values, handleBlur, handleChange, errors, touched, setFieldValue, isSubmitting, handleSubmit }) => (
          <Form>
            ...
            {values.contact_id !== values.sup_id && values.dialog && (
              <AlertCheckUpdate
                open={values.dialog}
                close={() => setFieldValue('dialog', false)}
                setFieldValue={setFieldValue}
                isSubmitting={isSubmitting}
