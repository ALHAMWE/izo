...
import PurchaseEntry from 'src/@core/components/purchases/entry/PurchaseEntry'
  ...
  return (
    <Fragment>
      ...
      >
        ...
        {/* Update status */}
        {purchaseStatus[status].title!=='Received'&&(
          <MenuItem
          onClick={() => {
            handleRowOptionsClose()
      ...
      {openEntry && <PurchaseEntry open={openEntry} toggle={handleOpenEntry} id={id} />}
    </Fragment>
  )
}
  ...
  return (
    <>
      <Chip
        ...
        color={status.color}
        onClick={() => {
          if(status.title!=='Received'){
            setOpenUpdateStatus(true)
          }
        }}
        sx={{
          cursor: status.title === 'Received' ? 'default' : 'pointer',
        }}
      
        
      />
      {openUpdateStatus &&(<UpdateStatus open={openUpdateStatus} toggle={toggle} id={row.id} />)}
    </>
  )
}
...
const ListPurchases = () => {
  ...
  const columns = [
    {
      field: 'actions', // name form backend
      headerName: 'Actions', // name to show in front
      flex: 0.2, // max is 1 and min is 0
      align: 'center', // left or right , center
      minWidth: 90, // in px
      renderCell: ({ row }) => <RowOptions id={row.id} status={row.status} /> // response for overrid i can write it or not
    },
    {
      field: 'date',
      headerName: 'Date',
    ...
    },
    {
      ...
      renderCell: ({ row }) => <CustomStatus row={row} />
    },
    {
